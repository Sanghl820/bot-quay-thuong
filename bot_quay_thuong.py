...
async def quay_thuong(context: CallbackContext):
    await context.bot.send_message(chat_id=group_id, text="🎲 Chương trình quay thưởng may mắn chính thức bắt đầu!\n\n🔒 Trong thời gian quay thưởng, group sẽ tạm cấm chat!")
    await context.bot.set_chat_permissions(chat_id=group_id, permissions=ChatPermissions(can_send_messages=False))
    if not participants:
    ...
    await context.bot.send_message(chat_id=group_id, text=result_text, parse_mode='HTML')
    ...
    if intro_message_id and group_id:
        try:
            if os.path.exists(INTRO_FILE):
                ...
                intro_text = "🎯 Chào mừng! Hãy nhấn Tham Gia để bắt đầu."
            full_intro = f"{intro_text}\n\n👥 Đã có {len(participants)} người tham gia"
            await context.bot.edit_message_text(chat_id=group_id, message_id=intro_message_id, text=full_intro, reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("✅ Tham Gia", callback_data="thamgia")]]))
        except Exception as e:
            print(f"⚠️ Lỗi update intro: {e}")

@only_admin
async def unlock_group(update: Update, context: CallbackContext):
    await context.bot.set_chat_permissions(chat_id=update.message.chat_id, permissions=ChatPermissions(can_send_messages=True))
